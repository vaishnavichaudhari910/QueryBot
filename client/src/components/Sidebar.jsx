...
const Sidebar = ({isMenuOpen,setIsMenuOpen}) => {
  const { chats, setSelectedChat, theme, setTheme, user,navigate ,createNewChat,axios,setChats,setToken,token,fetchUserChats} = useAppContext();
...
/>
       ...
       <div className="flex flex-col text-sm">
        ...
        <p className="text-xs text-gray-400">
              Purchase credits to use QueryBot
        </p>
       </div>
        </div>
{/* Dark Mode Toggle  */}
       ...
       <label onChange={()=>setTheme(theme === 'dark'? 'light':'dark')} className="relative inline-flex cursor-pointer">
        ...
        {/* <img onClick={()=>setIsMenuOpen(false)} src={assets.close_icon} className="absolute top-3 right-3 w-5 h-5 cursor-pointer md:hidden not-dark:invert"alt=""  onClick={e=>toast.promise(deleteChat(e,chat._id),{loading:'Deleting chat'})} /> */}
